...
var log = new Log('debug', fs.createWriteStream('log-' + currentDateTime + '.log')), hits = new Log('debug', fs.createWriteStream('hints-' + currentDateTime + '.log'));
...
class Photo {
    constructor(public name, public file_id) {
    }
}

var doggo_ids=[]

interface StructFunction{
    map:Map<String,String>
}

class DoggoBot {
    myID:number
    votetable:Set<Number>
    uploadtable:Array<Number>
    advicestable:Array<Number>
    bot
    photos
    text:TextUtils
    replyKeyboardMain:String
    cancelKeyboard:String
    fnmap
    
    private constructor(botinstance) {
        this.myID = 67447150;
        this.votetable = new Set()
        this.uploadtable = []
        this.advicestable = []
        this.bot = botinstance
        this.photos = PhotoUtils.GetInstance(botinstance, Entry, doggo_ids, this)
        this.text = TextUtils.GetInstance(botinstance)
        ...
        if (this.uploadtable.indexOf(fromid) != -1) {
            ...
            doggo.save().then(function (doc) {
                if (!doc) {
                    ...
                    log.debug('meow');
                    this.bot.sendPhoto(this.myID, msg.photo[msg.photo.length - 1].file_id, {
